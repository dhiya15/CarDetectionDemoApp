...
public class MultiBoxTracker {
  ...
  public synchronized void draw(final Canvas canvas) {
    ...
    RectF rect = new RectF(350, 0, 350, 480);
    getFrameToCanvasMatrix().mapRect(rect);
    canvas.drawRoundRect(rect, 0, 0, boxPaint2);
  ...
  private void processResults(final List<Recognition> results) {
    ...
    for (final Pair<Float, Recognition> potential : rectsToTrack) {
      ...
      trackedRecognition.title =
                      " w = " +
                      (int) (trackedRecognition.location.bottom-trackedRecognition.location.top) +
                      ", h = " +
                      (int) (trackedRecognition.location.right-trackedRecognition.location.left) +
                      ", x = " + (int) trackedRecognition.location.top +
                      ", y = " + (int) trackedRecognition.location.left
                      ;
